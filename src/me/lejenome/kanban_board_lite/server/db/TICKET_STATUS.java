...
public class TICKET_STATUS {
    ...
    public static void initialize() {
        try (ResultSet res = Connection.executeQuery("Select * From TICKET_STATUS")) {
            vals = new HashMap<>();
            while (res.next()) {
    ...
    public static boolean add(int level, String title) {
        boolean res = false;
        try {
            res = Connection.execute("Insert INTO TICKET_STATUS (level, title) Values (?, ?)",
    ...
    public static boolean delete(int level) {
        boolean res = false;
        try {
            res = Connection.execute("DELETE FROM TICKET_STATUS WHERE level = ?",
                    level, Types.SMALLINT);
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public static boolean update(int level, String title) {
        boolean res = false;
        try {
            res = Connection.execute("UPDATE TICKET_STATUS SET title = ? where level = ?",
                    title, Types.VARCHAR,
                    level, Types.SMALLINT);
        } catch (SQLException e) {
            e.printStackTrace();
